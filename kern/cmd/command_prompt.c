	...
	while (1)
	{
		...
		{ // Tab button
			if (last_c != 9)
			{
				...
				{
					if ((strcmp(arguments[0], "run") != 0) && (strcmp(arguments[0], "load") != 0) && (strcmp(arguments[0], "tst") != 0)) // to autocomplete only in case that the command take arguments and defined arguments (run & load & tst) only
						continue;
...
int process_command(int number_of_arguments, char **arguments)
{
	...
	if (command_found == 0)
	{
		...
		int Argument_len = strlen(arguments[0]);

		for (i = 0; i < NUM_OF_COMMANDS; i++)
		{
			int command_len = strlen(commands[i].name);
			int k = 0, j = 0;

			while (k < Argument_len && j < command_len)
			{
...
int initialize_prirrScheduler(int number_of_arguments, char **arguments)
{
