...
void test_initialize_dynamic_allocator()
{
	...
	*(tmp_ptr + 1) = 1;
	...
	{
		if (*daBeg != 1 || *daEnd != 1)
			cprintf("*daBeg != 1 || *daEnd != 1");
		if ((*blkHeader != initAllocatedSpace - 2 * sizeof(int)))
			cprintf("(*blkHeader != initAllocatedSpace - 2*sizeof(int))");
		if ((*blkFooter != initAllocatedSpace - 2 * sizeof(int)))
			cprintf("(*blkFooter != initAllocatedSpace - 2*sizeof(int))");
