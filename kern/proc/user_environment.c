...
void *create_user_kern_stack(uint32 *ptr_user_page_directory)
{
	...
	void *address = kmalloc(KERNEL_STACK_SIZE);
	if (address != NULL)
	{
		uint32 *ptr_page_table = NULL;
		get_page_table(ptr_user_page_directory, (uint32)address, &ptr_page_table);
		if (ptr_page_table != NULL)
		{
			ptr_page_table[PTX((uint32)address)] = ptr_page_table[PTX((uint32)address)] & (~PERM_PRESENT);
		}
		// cprintf("\naddress %p\n", address);
		return address;
	}
