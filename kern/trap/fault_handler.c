...
void fault_handler(struct Trapframe *tf)
{
	...
	else
	{
		if (userTrap)
		{
			...
			if ((page[PTX(fault_va)] & PERM_PRESENT) == PERM_PRESENT)
			{
				if ((fault_va >= USER_LIMIT))
				{
					env_exit();
				}
				else if (((page[PTX(fault_va)] & PERM_WRITEABLE) != PERM_WRITEABLE))
				{
					env_exit();
				}
			}

			else if ((page[PTX(fault_va)] & PERM_AVAILABLE) != PERM_AVAILABLE && (fault_va >= USER_HEAP_START && fault_va < USER_HEAP_MAX))
			{
				env_exit();
			}
...
void page_fault_handler(struct Env *faulted_env, uint32 fault_va)
{
	...
	if (wsSize < (faulted_env->page_WS_max_size))
	{
		...
		struct WorkingSetElement *WSele = env_page_ws_list_create_element(faulted_env, fault_va);

		LIST_INSERT_TAIL(&faulted_env->page_WS_list, WSele);
	...
	else
	{
		...
		int sweeps = (page_WS_max_sweeps < 0 ? -page_WS_max_sweeps : page_WS_max_sweeps);
		bool isNegative = (page_WS_max_sweeps < 0) ? 1 : 0;

		while (1)
		{
			if (victimWSElement == NULL)
				victimWSElement = LIST_FIRST(&faulted_env->page_WS_list); // reloop
			...
			if ((perms & PERM_USED) == 0)
			{
				...
				if (victimWSElement->sweeps_counter >= sweeps)
				{

					if ((perms & PERM_MODIFIED) != 0)
					{
						if (!isNegative || victimWSElement->sweeps_counter > sweeps)
						{
							uint32 *_;
					...
					bool is_vic_last =
						(victimWSElement == faulted_env->page_last_WS_element) ? 1 : 0;
					int temp = pt_get_page_permissions(
