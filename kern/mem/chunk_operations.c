...
void *sys_sbrk(int numOfPages)
{
	...
	if ((env->env_seg_break + (numOfPages * PAGE_SIZE)) < env->env_limit)
	{
		uint32 old = env->env_seg_break;
		allocate_user_mem(env, env->env_seg_break, (numOfPages * PAGE_SIZE));
		env->env_seg_break += (numOfPages * PAGE_SIZE);

		return (void *)(old);
	}
	return (void *)-1;*/
	uint32 size = numOfPages * PAGE_SIZE;
	uint32 hard_limit = env->env_limit;
	uint32 old_s_brk = env->env_seg_break;
	uint32 new_s_brk = old_s_brk + size;

	if (size == 0)
		return (void *)old_s_brk;
	if (new_s_brk > env->env_limit)
		return (void *)-1;
	else
	{
		allocate_user_mem(env, old_s_brk, size);
	}
	env->env_seg_break = new_s_brk;
	return (void *)old_s_brk;
}
...
void allocate_user_mem(struct Env *e, uint32 virtual_address, uint32 size)
{
	...
	for (uint32 i = 0; i < np; i++)
	{
		...
		pt_set_page_permissions(e->env_page_directory, virtual_address, (PERM_AVAILABLE | PERM_USER | PERM_WRITEABLE), 0);
