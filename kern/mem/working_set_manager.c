...
inline struct WorkingSetElement *env_page_ws_list_create_element(struct Env *e, uint32 virtual_address)
{
	...
	struct WorkingSetElement *ptr_WS_element = (struct WorkingSetElement *)kmalloc(sizeof(struct WorkingSetElement));
	if (ptr_WS_element == NULL)
		panic("Failed to allocate element for working set!");
	ptr_WS_element->virtual_address = virtual_address;
	// cprintf("\nptr_WS_element  %p\n", ptr_WS_element);

	uint32 *_;
	get_frame_info(e->env_page_directory, virtual_address, &_)->element = ptr_WS_element;
