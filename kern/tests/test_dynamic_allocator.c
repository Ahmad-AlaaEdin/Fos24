...
int check_block(void *va, void *expectedVA, uint32 expectedSize, uint8 expectedFlag)
{
	...
	cprintf("expectedSize %d", expectedSize);
	cprintf("fffffoooootetr %x", (uint32 *)(va + expectedSize - 8));
...
int test_initial_alloc(int ALLOC_STRATEGY)
{
	...
	if (is_correct)
		is_correct = check_block(va, expectedVA, remainSize, 1);
	if (is_correct)
		is_correct = check_list_size(0);
...
#define numOfFFTests 3
	...
	if (is_correct)
		is_correct = check_block(va, expectedVA, 2 * kilo, 1);
	if (is_correct)
		is_correct = check_list_size(numOfAllocs - 1);
	...
	*(tstEndVAs[numOfFFTests]) = 353;
	...
	if (is_correct)
		is_correct = check_block(va, expectedVA, allocSizes[1], 1);
	if (is_correct)
		is_correct = check_list_size(numOfAllocs - 2);
...
#define numOfBFTests 5
	...
	if (is_correct)
		is_correct = check_block(va, expectedVA, kilo / 4, 1);
	if (is_correct)
		is_correct = check_list_size(numOfAllocs - 1);
	...
	*(tstEndVAs[numOfBFTests]) = 353;
	...
	if (is_correct)
		is_correct = check_block(va, expectedVA, allocSizes[5], 1);
	if (is_correct)
		is_correct = check_list_size(numOfAllocs - 2);
...
void test_free_block_FF()
{
	...
	*(endVAs[idx]) = idx;
	...
	if (is_correct)
		is_correct = check_block(startVAs[numOfAllocs * allocCntPerSize], startVAs[numOfAllocs * allocCntPerSize], remainSize, 0);
	...
	expected_va = (curVA + sizeOfMetaData / 2);
	if (is_correct)
		is_correct = check_block(va, expected_va, remainSize, 1);

	// Free block before last
	free_block(startVAs[numOfAllocs * allocCntPerSize - 1]);
	if (is_correct)
		is_correct = check_block(startVAs[numOfAllocs * allocCntPerSize - 1], startVAs[numOfAllocs * allocCntPerSize - 1], allocSizes[numOfAllocs - 1], 0);
	...
	if (is_correct)
		is_correct = check_block(va, expected_va, allocSizes[0], 1);

	// Free 2nd block
	free_block(startVAs[1]);
	if (is_correct)
		is_correct = check_block(startVAs[1], startVAs[1], allocSizes[0], 0);
